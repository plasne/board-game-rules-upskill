...
# run the eval flow
EVAL_OUTPUT=$(pf run create --flow ./board-game-chat-eval/flow.dag.yaml --data ./evaluations/questions.jsonl --column-mapping question='${data.query}' context='${run.outputs.context}' answer='${run.outputs.answer}' --run $INFERENCE_ID)
